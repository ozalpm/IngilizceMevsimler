...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		public RozerinButton mobileMoveLeft;
		public RozerinButton mobileMoveRight;
		private void Update()
		{
			// horizontal input
			moveInput = InputSystem.HorizontalRaw();
			
			if (mobileMoveLeft.val+mobileMoveRight.val!=0)
			{
				moveInput = mobileMoveRight.val - mobileMoveLeft.val;
			}
		...
		public void Jump()
		{
			if(m_extraJumps > 0 && !isGrounded && !m_wallGrabbing)	// extra jumping
			{
				...
				GameManager.Init.PlayOneShoot(jumpClip);
			}
